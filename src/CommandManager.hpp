...
class CommandSetBase {
  ...
  public:
    ...
    bool parse(char* data, size_t data_name_len) {
        if (data_name_len != name_len) {
            return false;
        }

        size_t commandTitleLen = 0;
        ...
        if (std::memcmp(data, name, name_len) == 0) {
            ...
            data += data_name_len + 1;
            for (int i = 0; i < getCount(); i++) {
                if (getCommand(i)->parse(data, commandTitleLen)) {
                    return true;
                }
            }
        }
    ...
    virtual CommandBase* getCommand(int index) = 0;

    CommandSetBase(const char* name, const char* description) : name(name), name_len(std::strlen(name)), description(description), description_len(strlen(description)) {
    }
};

template<int count> class CommandSet : CommandSetBase {
  public:
    using CommandContainer = std::array<CommandBase*, count>;
    ...
    virtual CommandBase* getCommand(int index) {
