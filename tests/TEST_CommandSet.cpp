...
TEST(COMMANDSET, match) {
    Command myCommand("myCommand", "desc", +[](CommandBase& cmd){(void)cmd; });
    ...
    EXPECT_TRUE(commandSet.match("new_cmd", 7, true));
}

TEST(COMMANDSET, parse) {
    Command command1("command1", "desc", +[](CommandBase& cmd){(void)cmd; });
    Command command2("command2", "desc", +[](CommandBase& cmd){(void)cmd; });
    ...
    Command command3("command3", "desc", +[](CommandBase& cmd){(void)cmd; });
